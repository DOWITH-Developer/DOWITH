...
const changeCalendar = (id) => {
    const challenge = document.querySelector(`.result-${id}`);
    const previousDiv = challenge.previousElementSibling;
    const challengeContent = previousDiv.firstChild.textContent + '🤔';
    
    const newTag = document.createElement('p');
    newTag.classList.add('done');
    newTag.textContent = challengeContent;
    todaySelector.append(newTag)
}
